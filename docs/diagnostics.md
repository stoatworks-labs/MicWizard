...
The module is **vendored**, not shared: ``src/main/diag/``. Every repo in the fleet
...
| Platform | Directory |
| --- | --- |
| macOS | `~/Library/Logs/micwizard/` |
| Linux | `$XDG_STATE_HOME/micwizard/logs/` (default `~/.local/state/...`) |
| Windows | `%LOCALAPPDATA%\micwizard\logs\` |

`MICWIZARD_LOG_DIR` overrides it. The path is printed on the first line of every
...
`micwizard.YYYY-MM-DD.log`, rotated daily, seven kept, no colour escapes.

Verbosity comes from `MICWIZARD_LOG`, default `info`.
...
An Electron app is several processes, so `render-process-gone` and
`child-process-gone` are hooked too. A renderer that dies takes the window with
it and leaves the main process running and cheerful; neither raises anything
`uncaughtException` can see.
...
```bash
micwizard --collect-diagnostics
```

Writes `micwizard-diagnostics-<timestamp>.json` and prints its path. One file, so
