# Running unsigned MicWizard builds
...
MicWizard listens on the network, so the first time it starts Windows shows:

> **Windows Defender Firewall has blocked some features of MicWizard**
...
domain. MicWizard needs this to discover Shure and Sennheiser receivers and receive AES67 audio for headphone cue.

If you deny it (or the prompt appears behind another window and times out), the receiver list stays empty and metering never starts
...
**Private**. If MicWizard isn't listed, **Allow another app…** → **Browse** to the `.exe`.

Or from an elevated PowerShell:

```powershell
New-NetFirewallRule -DisplayName "MicWizard" -Direction Inbound -Program "C:\Path\To\MicWizard.exe" -Action Allow -Profile Private,Domain
```

To remove a bad rule and get the prompt back on next launch:

```powershell
Get-NetFirewallRule -DisplayName "*MicWizard*" | Remove-NetFirewallRule
```
...
```sh
codesign -dv --verbose=4 "/Applications/MicWizard.app"
spctl -a -vv "/Applications/MicWizard.app"   # still reports "rejected" — ad-hoc is not notarization
```
